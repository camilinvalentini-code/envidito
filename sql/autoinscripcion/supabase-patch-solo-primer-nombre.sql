-- La página pública (clasificatoria, "¿cuál es tu equipo?") mostraba el
-- nombre completo de cada jugador debajo del equipo. Es una página sin
-- login, así que de acá en más solo se guarda el primer nombre en el
-- campo público — nunca apellido. El nombre completo sigue existiendo,
-- privado, en players/team_players para el organizador.
...
begin
  ...
  for item in select * from jsonb_array_elements(p_jugadores) loop
    ...
    if coalesce(v_item_name, '') <> '' then
      ...
      -- Antes guardaba v_item_name (nombre completo). Ahora solo el
      -- primer nombre para el campo público.
...
-- Corrige a los equipos que ya quedaron con nombre completo expuesto
-- (los que se autoinscribieron antes de este cambio). Solo toca
-- equipos con jugadores estructurados en team_players — nunca el texto
-- libre que haya escrito el organizador a mano.
update teams t
